	...
	<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
		...
		<body>
			...
			<% response.setHeader("Cache-control", "no-cache, no-store, must-revalidate" );
				...
				if(session.getAttribute("logged_in")==null) { response.sendRedirect("index.html"); } %>
				...
				<div class="container-fluid px-0">
					...
					<div class="container vh-100 d-flex align-items-center justify-content-center">
						...
						<form action="edituser" method="POST" id="edituserinfo" class="form shadow p-3 rounded">
							...
							<input type="text" class="form-control mb-3" name="username" id="username"
								placeholder="Enter username" value="${sessionScope.user.getUsername()}" required>
							<input type="email" class="form-control mb-3" name="email" id="email"
								placeholder="Enter email id" value="${sessionScope.user.getEmailId()}" required>
							<input type="password" class="form-control mb-3" name="oldpassword" id="oldpassword"
								placeholder="Enter Old Password" required>

							<!-- -----------Change password----------- -->

							<a href="#" id="link-change-password" class="link-primary text-center d-block mb-3"
							...
							<div id="change-password" class="d-none">
								...
								<input type="password" class="form-control mb-3" id="reenteredpassword"
									placeholder="Re-Enter New Password">
								<a href="#" id="keep-old-password" class="link-primary text-center d-block mb-3"
									onclick="keepOldPassword()">keep Old Password</a>
							</div>

							<!-- ---------------------------------- -->

							<input type="submit" value="Edit" class="btn btn-sm btn-primary d-block mx-auto px-3"
				...
				<script>
					function showChangePassword() {
						document.getElementById("link-change-password").classList.add("d-none");
						document.getElementById("change-password").classList.remove("d-none");
					}
					function keepOldPassword() {
						document.getElementById("link-change-password").classList.remove("d-none");
